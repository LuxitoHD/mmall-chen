...
class PhocaGalleryCpModelPhocaGalleryProduct extends JModelAdmin
{
	...
	public function getTable($type = 'PhocaGalleryProduct', $prefix = 'Table', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true) {
		...
		$form 	= $this->loadForm('com_phocagallery.phocagalleryproduct', 'phocagalleryproduct', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
		return $form;
	...
	function save($data) {
		...
		if ((int)$table->id > 0) {
			PhocaGalleryTag::storeProductTags($data['tags'], (int)$table->id);
		}
	...
	function delete($cid = array()) {
		...
		if (count( $cid )) {
			...
			$queryd = 'SELECT filename as filename FROM #__phocagallery_products WHERE id IN ( '.$cids.' )';
			$this->_db->setQuery($queryd);
			$fileObject = $this->_db->loadObjectList();
			// - - - - - - - - - - - - - 

			//Delete it from DB
			$query = 'DELETE FROM #__phocagallery_products'
				. ' WHERE id IN ( '.$cids.' )';
...
//			}
	...
	protected function batchTag($value, $pks, $contexts)
	{
		...
		foreach ($value as $categoryId){
			foreach ($pks as $pk)
			{
				$query = 'DELETE FROM #__phocagallery_tags_products_ref'
				...
				$query = 'insert into #__phocagallery_tags_products_ref(imgid,tagid) values('
					. $pk .', '.$categoryId.')';
				$this->_db->setQuery( $query );
				if(!$this->_db->query()) {
					$this->setError($this->_db->getErrorMsg());
