...
class PhocaGalleryCpViewPhocaGalleryProduct extends JView
{
	protected $state;
	protected $item;
	protected $form;
	...
	protected function addToolbar() {
		...
		JToolBarHelper::title(   JText::_( 'COM_PHOCAGALLERY_PRODUCT' ).': <small><small>[ ' . $text.' ]</small></small>' , 'image');

		// If not checked out, can save the item.
		if (!$checkedOut && $canDo->get('core.edit')){
			JToolBarHelper::apply('phocagalleryproduct.apply', 'JTOOLBAR_APPLY');
			JToolBarHelper::save('phocagalleryproduct.save', 'JTOOLBAR_SAVE');
			JToolBarHelper::addNew('phocagalleryproduct.save2new', 'JTOOLBAR_SAVE_AND_NEW');
		...
		if (empty($this->item->id))  {
			JToolBarHelper::cancel('phocagalleryproduct.cancel', 'JTOOLBAR_CANCEL');
		}
		else {
			JToolBarHelper::cancel('phocagalleryproduct.cancel', 'JTOOLBAR_CLOSE');
		}
