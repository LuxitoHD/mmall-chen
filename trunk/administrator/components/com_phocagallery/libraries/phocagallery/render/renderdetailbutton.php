...
class PhocaGalleryRenderDetailButton
{
	...
	*/
	function getJsSlideshow($catid, $id, $slideshow = 0, $catidSlug, $idSlug) {
		...
		$query = 'SELECT a.id,a.title, a.filename, a.extl, a.description, a.hits, a.bads, a.goods ' 
		.' FROM #__phocagallery AS a'
		...
		if (!empty($filenameAll)) {
			...
			foreach ($filenameAll as $key => $value) {
				...
				$tags =PhocaGalleryTag::displayTags_th($value->id);
				...
				if (isset($value->extl) && $value->extl != '') {
					...
					if (JFile::exists($fileThumbnail->abs)) {
						$jsSlideshowData['files'] .= '["'. $imgLink .'", "'. $imgLink_small .'", "'.$value->title.'", "'.$tags.'" , "'.$description.'", "'.$value->hits.'", "'.$value->goods.'", "'.$value->bads.'", "'.$value->id.'"]'.$endComma."\n";
					} else {
